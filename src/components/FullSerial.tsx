...
import { selectFilm, selectLoading } from '../features/Movie/movieSlice';
import { loadSerial, selectSerial } from '../features/Serial/serialSlice';
...
const FullSerial: FC = () => {
  ...
  const data = useAppSelector(selectSerial)
  const trailerKey = results ? results.filter(item => item.type === 'Trailer')[0]?.key : 'qzbtdclsJXw'
  ...
  useEffect(() => {
    dispatch(loadTrailer({ type: 'tv', id: serialId }))
    dispatch(loadMovieActors({ movieId: Number(serialId), type: 'tv' }))
    dispatch(loadRecomendations({ type: 'tv', id: serialId }))
  }, [])


  useEffect(() => {
    dispatch(loadSerial(Number(serialId)))
    window.scrollTo(0, 0)
  }, [serialId])
  ...
  return (
    <>
      ...
      <Main isOpen={isOpen}>
        ...
        {isOpen && <YouTubeBlock>
          <YouTube videoId={trailerKey ? trailerKey : 'qzbtdclsJXw'} opts={opt} onReady={onPlayerReady} />
